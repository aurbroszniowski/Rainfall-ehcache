 ...
 */
public class ReplaceOperationFunction<K, V> implements OperationFunction<EhcacheResult> {

  private static final Logger log = LoggerFactory.getLogger(ReplaceOperationFunction.class);
  ...
  public ReplaceOperationFunction(final Cache<K, V> cache, final long next, final ObjectGenerator<K> keyGenerator,
  ...
  public EhcacheResult apply() throws Exception {
    ...
    try {
      ...
      log.debug("replace(k, v) operation failed.", e);
      return EXCEPTION;
    }
    if (v == null) {
      return MISS;
