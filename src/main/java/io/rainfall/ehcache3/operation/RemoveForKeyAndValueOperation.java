...
public class RemoveForKeyAndValueOperation<K, V> implements Operation {
  ...
  public RemoveForKeyAndValueOperation(final ObjectGenerator<K> keyGenerator, final ObjectGenerator<V> valueGenerator,
  ...
  public void exec(final StatisticsHolder statisticsHolder, final Map<Class<? extends Configuration>,
    ...
    for (final CacheDefinition<K, V> cacheDefinition : cacheDefinitions) {
      ...
      try {
        ...
        if (!removed) {
          statisticsHolder.record(cacheDefinition.getName(), (end - start), REMOVEVALUE_MISS);
        } else {
          statisticsHolder.record(cacheDefinition.getName(), (end - start), REMOVEVALUE);
        }
      } catch (Exception e) {
        long end = statisticsHolder.getTimeInNs();
